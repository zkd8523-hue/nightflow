...
const T = {
  ...
  "zh-tw": {
    h2: "桌位實際要花多少 — 酒單真實價格",
    intro: (n: number) =>
      `NightFlow 現在可訂桌的首爾夜店有${n}家。桌位按每桌整組的低消訂位（梨泰院、弘大 50萬韓元，江南 100萬韓元），這個金額由您從夜店酒單中選酒湊滿。下面是各夜店酒單上最便宜的項目，讓您看清這筆錢能買到什麼。這些是夜店自己的價格，NightFlow 不加收費用。`,
    minSpend: "每桌低消",
    fromLabel: "該店起價",
    andUp: "起",
    set: "最便宜套餐", item: "最便宜單瓶", book: "訂桌",
    charge: "桌位費", reviews: "則評論",
    note: "平日酒單價格（韓元），取自各夜店酒單。週末可能更高，有桌位費的夜店已在上方標註。",
    faqH2: "首爾夜店桌位價格 — 常見問題",
    jump: "跳至",
  },
} as const;
  ...
  if (lang === "zh-tw") {
    return [
      ...
      },
      {
        q: "桌位價格包含什麼？",
        a: ex.cheapest?.description
          ? `套餐內容以夜店酒單為準。例如${ex.name}的${ex.cheapest.name}（${exPrice}）包含「${ex.cheapest.description}」。顯示價格就是夜店自己的價格，無中介費。部分夜店另收桌位費。`

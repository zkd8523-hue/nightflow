        ...
        hideClubHeader
          ...
          : `animate-offer-card-enter relative rounded-2xl border p-4 space-y-3 ${
              offer.status === "accepted"
                ...
                : "bg-offer-well border-border"
            }`
      }
    >
      {hideClubHeader ? (
        (offer.status === "accepted" || showOfferNumber || !showsGoldCta) && (
        <div className="flex items-center justify-between">
          <span className="text-[13px] font-black text-foreground/80 tabular-nums">
            {offer.status === "accepted"
              ? t("✓ 매치됨", "✓ Matched")
              : showOfferNumber
          ...
          <div>
            {offer.club_id ? (
              <Link
                ...
                className="inline-flex items-baseline gap-0.5 text-[21px] font-black text-foreground hover:text-brand-amber transition-colors"
              >
                {club?.name || t("클럽", "Club")}
                <ChevronRight className="w-3.5 h-3.5 text-muted-foreground self-center" />
              </Link>
            ) : (
              <p className="text-[21px] font-black text-foreground">
                {club?.name || t("클럽", "Club")}
              </p>
            )}
          </div>
          {offer.status === "accepted" ? (
            <span className="text-[11px] px-2.5 py-1 rounded-full bg-amber-500/20 text-brand-amber font-bold">
              {t("✓ 매치됨", "✓ Matched")}
            </span>
          ) : club?.area ? (
            <span className="text-[12px] text-muted-foreground font-black">{areaLabel(club.area, lang)}</span>
          ) : (
            <span className="text-[11px] text-muted-foreground font-medium" suppressHydrationWarning>
              {formatRelativeTime(offer.created_at)}
            </span>
      ...
      <div className="space-y-2">
        ...
        {offer.comment && (
          <p className="text-[13px] leading-relaxed text-foreground/80 border-l border-border pl-3">
      ...
      {dealCount != null && dealCount >= 3 && (
        <div className="flex items-center gap-1.5 pt-2 border-t border-border/60">
          ...
          <span className="text-[10px] font-bold text-muted-foreground">{isForeigner ? `${dealCount} deals` : `거래 ${dealCount}회`}</span>
        </div>
      )}
          ...
          fallback={
            <div className="flex gap-2 pt-1">
              <Button
                ...
                className="flex-1 h-10 bg-inverse hover:opacity-90 text-inverse-foreground font-black text-[13px] rounded-xl"
              >
        ...
        >
          {isRecruitingParty ? (
            ...
            isAdmin ? (
              ...
              <Link
                ...
                className={`flex flex-col items-center justify-center gap-0.5 w-full mt-1 rounded-xl py-2 px-3 font-bold text-[13px] ${
                  offer.chat_meta?.replied
                    ...
                    : "bg-amber-500/15 border border-amber-500/40 text-brand-amber hover:bg-amber-500/25"
                }`}
              >
              ...
              <Link
                href={`/messages/${offer.id}`}
                className="flex items-center justify-center gap-1.5 w-full h-11 mt-1 rounded-xl bg-muted hover:bg-muted text-foreground font-bold text-[13px]"
              >
                <MessageCircle className="w-4 h-4" />
                {t("상담중", "Chatting")}
              </Link>
            ...
            /* 클럽 그룹 안 여러 MD 카드 — 빛나는 골드 칩형 CTA (주류 배지보다 위계 우선)
               시간은 별도 행 대신 이 행에 같이 붙여서 수직 공간을 아낌 */
        ...
        <div className="pt-2 mt-1 border-t border-red-500/20 bg-red-500/5 -mx-4 px-4 pb-1 space-y-1">
          ...
          <div className="flex items-center gap-3 text-[12px] text-foreground/80 flex-wrap">
            <span className="inline-flex items-center gap-1">
              ...
              <span className="font-bold text-foreground">{offer.md.display_name || "이름 없음"}</span>
            </span>
            {offer.md.instagram && (
              <a
        ...
        <div className="pt-1 flex justify-end gap-2">
          {onAdminEdit && (
            <button
              ...
              disabled={actionLoading}
              className="h-8 px-3 rounded-lg border border-border bg-transparent text-foreground/80 hover:bg-muted font-bold text-[12px] inline-flex items-center gap-1.5 disabled:opacity-50"
            >

/**
 * 클럽 인스타 게시물 → 출연 라인업 추출. 시스템 프롬프트 + 툴 정의.
 *
 * ⚠️ 이 파일이 원본이다. Edge Function(Deno)은 이 npm 경로를 import할 수 없으므로
 * supabase/functions/_shared/lineup-prompt.ts 에 동일한 내용을 복제해 둔다.
 * 두 파일이 갈라지면 수동/자동 파싱 결과가 미묘하게 달라지는데, 이건 몇 달 뒤에나
 * 발견되는 종류의 버그다. 이 파일을 고치면 반드시 Edge Function 쪽도 같이 고치고
 * `node scripts/check-lineup-prompt-sync.mjs` 로 확인할 것.
 *
 * 입력은 캡션 + (있으면) 포스터 이미지다. 관리자 수동 업로드는 이미지만 넘기는데,
 * 그때도 동작해야 한다.
 *
 * 12시간제 변환 규칙이 프롬프트의 큰 부분을 차지하는 이유: 클럽 포스터의
 * "10:00-11:00"은 밤 10시다. 이 변환을 모델에 안 시키면 운영자가 매 건 손으로 고쳐야 한다.
 */

...
//
// 어휘(2026-09-14 PTT·Dcard 실측으로 뒤집음): zh-tw는 包廂. 크리틱 1차 땐 "包廂=밀폐 VIP룸
// 어감"이라 桌位/訂桌을 골랐는데, 대만인 후기 원문이 한국 클럽의 오픈 부스를 그대로 包廂이라
// 부른다("外圍包廂的全是男生", "被抓進去包廂"). 訂桌은 실제로 거의 안 쓴다. 검색어는 包廂으로
// 잡되, 룸으로 오해하지 않게 첫 등장 자리(ClubBookingSection 인트로·RealTablePrices FAQ)에서
...
const COPY: Record<SeoLang, Copy> = {
  ...
  "zh-tw": {
    ...
    descPrice: (n, f, fc) => `${n}包廂低消${fc}(${f})起 — 真實酒單價格,中文預訂,無中介費。`,
    descAlt: (a) => `看看${a}現在就能訂包廂的夜店和真實包廂價格。`,
    faqPriceQ: (n) => `${n}的包廂多少錢？`,
    faqPriceA: (n, f, a, lowest, reachIn) =>
      lowest && lowest !== f
        ? `透過 NightFlow 在${n}訂包廂,每桌低消${f}(${a}區域統一的低消)。酒單上的酒水和套餐從${lowest}起` +

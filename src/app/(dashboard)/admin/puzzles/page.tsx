...
const STATUS_COLOR: Record<string, string> = {
  open: "bg-green-500/20 text-money",
  selecting: "bg-purple-500/20 text-purple-400",
  matched: "bg-amber-500/20 text-brand-amber",
  accepted: "bg-amber-500/20 text-brand-amber",
...
export default async function AdminPuzzlesPage({ searchParams }: PageProps) {
  ...
  const OFFER_STATUS_COLOR: Record<string, string> = {
    pending: "bg-amber-500/20 text-brand-amber",
    accepted: "bg-green-500/20 text-money",
          ...
          <div className="space-y-4">
            ...
            <div className="flex items-center gap-1.5">
              ...
              <Link href={buildHref({ kind: null, area: null, status: null })}>
                <button className={`px-3 py-1.5 rounded-full text-[12px] font-bold transition-all ${!kindFilter ? "bg-inverse text-inverse-foreground" : "bg-muted text-muted-foreground border border-border"}`}>
                  전체 {allPuzzles?.length || 0}
                </button>
              </Link>
              <Link href={buildHref({ kind: "flag", area: null, status: null })}>
                <button className={`px-3 py-1.5 rounded-full text-[12px] font-bold transition-all ${kindFilter === "flag" ? "bg-inverse text-inverse-foreground" : "bg-muted text-muted-foreground border border-border"}`}>
                  🚩 깃발 {flagCountAll}
                </button>
              </Link>
            ...
            {/* 정렬 토글 */}
            <div className="flex items-center gap-1.5">
              <span className="text-[11px] text-muted-foreground font-bold mr-1">정렬:</span>
                    ...
                    <div className="flex items-center gap-1.5 mb-1">
                      <Link href={buildHref({ chat: chatFilter === "waiting" ? null : "waiting" })} className="flex-1">
                        <div className={`flex items-center justify-center gap-1 py-2 rounded-lg text-[12px] font-bold transition-all ${
                          chatFilter === "waiting" ? "bg-amber-500 text-black" : "bg-muted text-foreground/80 border border-border"
                        }`}>
                          💬 답장대기중 {waitCnt}
                        </div>
                      </Link>
                      <Link href={buildHref({ chat: chatFilter === "active" ? null : "active" })} className="flex-1">
                        <div className={`flex items-center justify-center gap-1 py-2 rounded-lg text-[12px] font-bold transition-all ${
                          chatFilter === "active" ? "bg-green-500 text-black" : "bg-muted text-foreground/80 border border-border"
                        }`}>
                          🗨️ 대화중 {activeCnt}
                        </div>
                      </Link>
                ...
                <ProgressiveList initial={7} items={filteredPuzzles.map((puzzle) => {
                  ...
                  return (
                    <Fragment key={puzzle.id}>
                      ...
                      <Card className="bg-card border-border p-5 space-y-4 hover:border-border transition-colors">
                        {/* 퍼즐 기본 정보 (깃발 상세로 링크) */}
                        <Link href={`/flags/${puzzle.id}`} className="block">
                          <div className="flex items-start justify-between">
                            <div>
                              {puzzle.notes && (
                                <p className="text-[15px] font-black text-foreground">{puzzle.notes}</p>
                              )}
                              <p className={`${puzzle.notes ? "text-[13px] text-muted-foreground" : "text-[16px] font-black text-foreground"}`}>
                          ...
                          <div className="bg-amber-500/10 border border-amber-500/30 rounded-lg px-3 py-2">
                            <div className="flex items-center justify-between gap-2 flex-wrap">
                              <p className="text-[12px] text-brand-amber">
                                ...
                                <span className="text-brand-amber dark:text-brand-amber/70 mx-1">·</span>
                                <span className="font-black text-money">
                          ...
                          return (
                            <Link key={o.id} href={`/flags/${puzzle.id}`}
                              className={`flex flex-col gap-0.5 rounded-lg px-3 py-2 mt-1 ${
                                active ? "bg-muted hover:bg-muted" : "bg-amber-500/10 border border-amber-500/30 hover:bg-amber-500/20"
                              }`}>
                              <span className={`text-[12px] font-bold ${active ? "text-foreground" : "text-brand-amber"}`}>
          ...
          <div className="space-y-3">
            ...
            {(recentOffers || []).length === 0 ? (
              ...
              (recentOffers || []).map((offer) => {
                ...
                return (
                  <Card key={offer.id} className="bg-card border-border p-4">
                    ...
                    <div className="flex items-start justify-between gap-2">
                      ...
                      <span className={`text-[11px] font-bold px-2.5 py-1 rounded-full shrink-0 ${OFFER_STATUS_COLOR[offer.status] || "bg-muted text-muted-foreground"}`}>
                        {OFFER_STATUS_LABEL[offer.status] || offer.status}
                      </span>
                    </div>
                    {offer.comment && (
                      <p className="text-[12px] text-muted-foreground mt-2 bg-card rounded-lg px-3 py-2">
                        {offer.comment}
                      </p>
                    )}
                    </Link>
          ...
          <div className="space-y-4">
            {puzzleEntries.length === 0 ? (
              ...
              puzzleEntries.map(([puzzleId, puzzleReports]) => {
                ...
                return (
                  <Card key={puzzleId} className="bg-card border-border p-5 space-y-4">
                    <div className="flex items-center justify-between">
                      <div>
                        {puzzle ? (
                          ...
                          <p className="text-[13px] text-muted-foreground">깃발 ID: {puzzleId.slice(0, 8)}</p>
                        )}
                      </div>
                      <span
                    ...
                    <div className="space-y-2">
                      <p className="text-[12px] font-bold text-muted-foreground">신고 {reportList.length}건</p>
                      {reportList.map((report) => (
                        <div
                          key={report!.id}
                          className="bg-card rounded-lg px-3 py-2 text-[12px] text-foreground/80 flex items-start justify-between gap-2"
                        >
            ...
            <div className="space-y-5">
              ...
              <div className="bg-card rounded-2xl border border-border p-4 space-y-3">
                ...
                <div className="flex items-center gap-2 flex-wrap">
                  <span className="text-[11px] text-muted-foreground font-bold w-8">구분</span>
                  {[
                    { value: "", label: "전체" },
                ...
                {/* 사유 필터 */}
                <div className="flex items-center gap-2 flex-wrap">
                  <span className="text-[11px] text-muted-foreground font-bold w-8">사유</span>
                  {[
                    { value: "", label: "전체" },

...
export default function AdminSupportPage() {
  ...
  return (
    <div className="max-w-lg mx-auto min-h-dvh bg-background">
      <header className="flex items-center gap-2 px-3 py-3 border-b border-border">
        ...
        <h1 className="text-[16px] font-black text-foreground">고객 문의</h1>
      </header>
      ...
      {loading ? (
        ...
        <div className="flex flex-col items-center justify-center text-center py-24 gap-3">
          ...
          <p className="text-[14px] text-muted-foreground font-bold">아직 문의가 없어요</p>
        </div>
      ) : (
        <div className="divide-y divide-neutral-800/60">
          {threads.map((t) => (
            <Link
              ...
              className="flex items-center gap-3 px-4 py-3.5 hover:bg-card/40 transition-colors"
            >
              <div className="relative w-11 h-11 rounded-full overflow-hidden bg-muted shrink-0">
                {t.profile_image ? (
                  ...
                  <div className="w-full h-full grid place-items-center text-muted-foreground text-[13px] font-bold">
                    {t.user_name?.[0] ?? "?"}
                  </div>
                )}
                {t.unread && (
              ...
              <div className="flex-1 min-w-0">
                <div className="flex items-center justify-between gap-2">
                  <p className={`text-[14px] truncate ${t.unread ? "text-foreground font-black" : "text-foreground font-bold"}`}>
                    {t.user_name}
                  </p>
                ...
                <p className={`text-[12px] truncate ${t.unread ? "text-foreground/80" : "text-muted-foreground"}`}>

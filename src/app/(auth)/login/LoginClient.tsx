...
interface LoginClientProps {
  /** 서버에서 파싱된 lang. useSearchParams()의 Suspense fallback flash를 완전 회피. */
  lang: string | null;
  /** 서버에서 파싱된 redirect 경로 */
  redirectPath: string;
  /** 서버에서 파싱된 인증 오류 코드 */
  authErrorCode: string | null;
}

export function LoginClient({ lang, redirectPath, authErrorCode }: LoginClientProps) {
  ...
  const handleKakaoLogin = async (customRedirect?: string) => {
    ...
    // Google/Apple과 동일 패턴 — 외국인 로그인 시 target에 lang 유지.
    // 이전엔 카카오에서만 lang이 유실돼 회원가입 후 한국어 폼 노출 이탈 발생.
    ...
    try {
      ...
      if (Capacitor.isNativePlatform()) {
        ...
        if (isNewUser) {
          router.push(`/signup${targetWithLang !== "/" ? `?next=${encodeURIComponent(targetWithLang)}` : ""}`);
        } else {
          router.push(targetWithLang);
          router.refresh();
        }
        setLoading(false);
        return;
  ...
  const handleAppleLogin = async (customRedirect?: string) => {
    ...
    // 외국인 로그인 시 target에 lang 유지 (네이티브·웹 공통).
    const langToken = isForeigner ? (lang || "en") : null;
    const targetWithLang = langToken && !/[?&]lang=/.test(target)
      ? target + (target.includes("?") ? `&lang=${langToken}` : `?lang=${langToken}`)
      : target;

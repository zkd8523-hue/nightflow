...
// 라틴 표기 유지(2026-08-09 확인). 속성어(營業時間/入場費 등)만 번역.
// 반각 쉼표/괄호 사용 — 기존 /zh-tw 지역 페이지 원문 문체를 따름(간체판은 전각 사용).
...
export async function generateMetadata({
  ...
  const hours = club.operating_hours ? translateClubMeta(club.operating_hours, "zh-tw") : null;
  const fee = club.entry_fee_detail ? translateClubMeta(club.entry_fee_detail, "zh-tw") : null;

  const url = `https://nightflow.kr/zh-tw/clubs/${area}/${clubParam}`;
  // 예약 가능 + 가격이 있으면 title이 "Book a Table from ₩X"로 바뀐다(예약 의도 검색어 CTR).
  const seo = bookingSeoBits({ lang: "zh-tw", name, areaLabel: areaZh, areaKo: club.area, bookable, pricing: pricing.get(club.id) });

  const descBits = [
    `${name}是${area === "busan" ? "" : "首爾"}${areaZh}的一家夜店。`,
    // 비예약: "근처는 된다"를 평점 문장보다 앞에 — 뒤에 두면 160자 컷에 잘려 안 보였다(크리틱 2차)
    bookable ? null : seo.descAlt,
    hours ? `營業時間：${hours}。` : null,
    fee ? `入場費：${fee}。` : null,
    club.google_rating ? `Google 評分 ${club.google_rating.toFixed(1)}（${club.google_review_count ?? 0} 則評論）。` : null,
  ].filter(Boolean);
  ...
  return {
    ...
    openGraph: {
      title: `${name} — ${areaZh}夜店${area === "busan" ? "" : "，首爾"}`,
      // 공유 카드에도 가격 훅(크리틱 3차)
...
export default async function ZhTwClubDetailPage({
  params,
}: {
  params: Promise<{ area: string; club: string }>;
}) {
  ...
  if (canonical && canonical !== area) {
    permanentRedirect(`/zh-tw/clubs/${canonical}/${clubParam}`);
  }
  ...
  const hours = club.operating_hours ? translateClubMeta(club.operating_hours, "zh-tw") : null;
  const fee = club.entry_fee_detail ? translateClubMeta(club.entry_fee_detail, "zh-tw") : null;
  const dress = club.dresscode ? translateClubMeta(club.dresscode, "zh-tw") : null;
  const features = clubFeatureLabels(club.tags, "zh-tw");
  // 평점 높은 순 — 구글이 주는 순서는 뒤죽박죽이라 첫 리뷰가 1점이면 바로 이탈한다.
  const reviews = (club.google_reviews ?? [])
    .filter((r) => r.text?.trim())
    .sort((a, b) => (b.rating ?? 0) - (a.rating ?? 0))
    .slice(0, 5);
  const googleUrl = getGoogleReviewsUrl({ name: club.name, address: club.address, area: club.area }, "zh-tw");
  const url = `https://nightflow.kr/zh-tw/clubs/${area}/${clubParam}`;
  const bookHref = `/flags/new?lang=zh-tw&area=${encodeURIComponent(club.area)}&club=${club.id}`;

  // ── 예약 SEO(2026-09-10): 실가격·Offer·대안 카드. 숫자는 폼 카드와 같은 규칙(tablePricing). ──
  const seoInput = { lang: "zh-tw" as const, name, areaLabel: areaZh, areaKo: club.area, bookable, pricing: pricing.get(club.id) };
  const seoBits = bookingSeoBits(seoInput);
  const fxRates = (await getKrwRates()).rates;
  const altClubs = bookableSiblings.map((s) => ({
    id: s.id,
    name: s.name_en!.trim(),
    href: `/zh-tw/clubs/${area}/${clubSlug(s.name_en!)}`,
    bookHref: `/flags/new?lang=zh-tw&area=${encodeURIComponent(s.area)}&club=${s.id}`,
  ...
  const faqs = [
    hours && { q: `${name}的營業時間是？`, a: `${name}（${areaZh}）的營業時間是${hours}。` },
    fee && { q: `${name}的入場費是多少？`, a: `${name}的入場費是${fee}。` },
    // 한글이 섞인 드레스코드("크록스 X" 등)는 번역이 안 된 원문이라 FAQ·JSON-LD에서 뺀다(크리틱 2차)
    dress && !/[가-힣]/.test(dress) && { q: `${name}有服裝規定嗎？`, a: `${name}的服裝規定：${dress}。` },
    club.address && { q: `${name}在哪裡？`, a: `${name}位於${areaZh}${area === "busan" ? "" : "（首爾）"}${club.address}。` },
  ...
  const jsonLd = {
    ...
    "@graph": [
      ...
      },
      {
        "@type": "BreadcrumbList",
        itemListElement: [
          { "@type": "ListItem", position: 1, name: `${area === "busan" ? "韓國" : "首爾"}夜店`, item: "https://nightflow.kr/zh-tw/clubs" },
          { "@type": "ListItem", position: 2, name: `${areaZh}夜店`, item: `https://nightflow.kr/zh-tw/clubs/${area}` },
    ...
    <div className="min-h-screen bg-background text-foreground pb-28 pb-safe">
      ...
      <div className="max-w-lg lg:max-w-[900px] mx-auto px-4 lg:px-8 py-6 lg:py-10 space-y-8">
        <nav className="flex items-center gap-1.5 text-[12px] text-muted-foreground flex-wrap">
          <Link href="/zh-tw/clubs" data-nf-track="breadcrumb_index" className="hover:text-foreground">{`${area === "busan" ? "韓國" : "首爾"}夜店`}</Link>
          <span>/</span>
        ...
        <header className="space-y-2">
          ...
          <p className="text-muted-foreground text-[14px]">
            {areaZh}{area === "busan" ? "" : "（首爾）"}夜店{club.name !== name && <> ・ {club.name}</>}
          </p>
          {club.google_rating != null && (
            <a href={googleUrl} target="_blank" rel="noopener noreferrer" data-nf-track="outbound_google_rating"
              ...
              <span className="text-muted-foreground">
                ・{(club.google_review_count ?? 0).toLocaleString()} 則 Google 評論
              </span>
            </a>
          )}
        </header>
        ...
        {club.thumbnail_url && (
          <div className="relative w-full h-56 rounded-2xl overflow-hidden">
            <Image src={club.thumbnail_url} alt={`${name} — ${areaZh}夜店${area === "busan" ? "" : "，首爾"}`} fill
              className="object-cover" sizes="(max-width: 640px) 100vw, 512px" priority />
          </div>
        )}

        <section>
          <h2 className="text-[18px] font-black mb-1">{name} — 營業時間・入場費・地址</h2>
          <dl className="rounded-2xl bg-card border border-border px-4">
            {hours && fact(<Clock className="w-4 h-4" />, "營業時間", hours)}
            {fee && fact(<Ticket className="w-4 h-4" />, "入場費", fee)}
            {club.address && fact(<MapPin className="w-4 h-4" />, "地址", club.address)}
            {dress && fact(<Shirt className="w-4 h-4" />, "服裝規定", dress)}
        ...
        {features.length > 0 && (
          <section>
            <h2 className="text-[18px] font-black mb-2">音樂・類型</h2>
        ...
        {reviews.length > 0 && (
          <section>
            <h2 className="text-[18px] font-black mb-2">{name}的評價</h2>
        ...
        {faqs.length > 0 && (
          <section>
            <h2 className="text-[18px] font-black mb-2">{name}常見問題</h2>
      ...
      <div className="fixed bottom-0 inset-x-0 lg:left-[248px] z-10 px-4 pt-3 pb-4 pb-safe bg-card/95 backdrop-blur-sm border-t border-border">
        ...
        {bookable && <UrgencyLine lang="zh-tw" align="left" className="w-full max-w-lg lg:max-w-[900px] mx-auto mb-2 px-0.5" />}

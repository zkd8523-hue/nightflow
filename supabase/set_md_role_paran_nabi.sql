...
BEGIN
  ...
  FROM users
  WHERE lower(display_name) = lower('파란나비')
    AND deleted_at IS NULL;          -- soft delete 된 계정 제외

  IF v_count = 0 THEN
    RAISE EXCEPTION '닉네임 "파란나비" 유저를 찾을 수 없습니다.';
  ELSIF v_count > 1 THEN
    RAISE EXCEPTION '닉네임 "파란나비" 유저가 % 명입니다. 수동 확인 필요 (id로 직접 지정하세요).', v_count;
  END IF;

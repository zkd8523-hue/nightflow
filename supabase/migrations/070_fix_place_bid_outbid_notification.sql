-- Migration 070: place_bid() 통합 수정 - outbid 인앱 알림 + 회귀 버그 복원
-- 날짜: 2026-03-16
-- 문제: Migration 060/062에서 place_bid()를 재작성하면서 054의 기능 누락
--   - previous_bidder_id 반환 누락 → 클라이언트 outbid 알림 API 호출 불가
--   - NULL 체크, scheduled→active 자동 전환, 연속 입찰 방지, extension 제한 누락
-- 해결: 054 + 060/062의 모든 기능 통합 + outbid 인앱 알림을 DB에서 직접 생성
...
COMMENT ON FUNCTION place_bid(UUID, UUID, INTEGER) IS 'Migration 070: 054+060+062 통합 + outbid 인앱 알림 DB 직접 생성';

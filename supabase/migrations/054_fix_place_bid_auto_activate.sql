-- Migration 054: place_bid() 회귀 버그 수정
-- 날짜: 2026-03-10
-- 문제: Migration 030/047에서 place_bid()를 재작성하면서 아래 5가지 기능이 누락됨
--   1. 경매 NULL 체크 (015에서 추가)
--   2. scheduled→active 자동 전환 (015에서 추가)
--   3. 연속 입찰 방지 (016에서 추가)
--   4. previous_bidder_id 반환 (017에서 추가)
--   5. extension_count/max_extensions 연장 제한 (031에서 추가)
-- 기존 유지: SECURITY DEFINER, Model B, contact_deadline, BIN, blocked_until, bidder_count, auto-extend
...
BEGIN
  ...
  -- 3. scheduled 경매 자동 활성화: 시작 시간이 지났으면 active로 전환
  ...
  -- 15. 자동 연장 (마감 auto_extend_min 이내 입찰 시, 횟수 제한 적용)
...
COMMENT ON FUNCTION place_bid(UUID, UUID, INTEGER) IS 'Migration 054: 자동 활성화 + 연속 입찰 방지 + previous_bidder_id + extension 제한 복원';

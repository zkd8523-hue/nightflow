-- 025: 결제 보장 시스템
-- 1) place_bid() blocked_until 시한부 차단 체크 추가
-- 2) notification_logs event_type에 'payment_reminder' 추가

-- 1. notification_logs event_type 확장
...
ALTER TABLE notification_logs ADD CONSTRAINT notification_logs_event_type_check
  CHECK (event_type IN (
    'auction_started','auction_won','payment_completed',
    'visit_confirmed','outbid','payment_reminder'
  ));

-- 2. place_bid() 재정의: blocked_until 시한부 차단 체크 추가

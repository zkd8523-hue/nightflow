-- Migration 082: place_bid()에 탈퇴 유저 입찰 차단 가드 추가
-- 날짜: 2026-04-07
-- 기반: Migration 076 (place_bid + MD 입찰 알림)
-- 문제: 탈퇴(soft delete) 처리된 유저가 비보호 경로의 경매 상세에서 입찰 시도 가능
--      → middleware는 보호 경로만 deleted_at 체크, RPC 수준 가드 부재
-- 해결: place_bid() 7번 단계 차단 검증에 deleted_at IS NOT NULL 추가
...
COMMENT ON FUNCTION place_bid(UUID, UUID, INTEGER) IS
  'Migration 082: 076 기반 + deleted_at 가드 추가 (탈퇴 유저 입찰 차단)';

-- ============================================================================
-- Migration 152: submit_offer / update_offer 에서 admin 역할도 제안 가능하게 허용
-- 날짜: 2026-05-11
-- 설명:
--   - 운영/테스트 편의를 위해 admin 계정도 깃발에 오퍼 제출 가능하도록 role 가드 확장
--   - md_status='approved' 체크는 role='md'일 때만 적용 (admin은 면제)
--   - 슬롯/일일 카운트 제한은 그대로 적용 (운영 한도 유지)
--   - 정가 검증(Migration 151) 등 기타 룰은 동일
-- 참조: Migration 127(price policy), 128(current_count), 138(update_offer), 151(정가 고정)
-- ============================================================================
